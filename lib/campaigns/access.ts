// Access control for the Campaigns module.
//
// Campaigns is a Practice-tier tool, so whether it appears for a firm at all is
// governed by the firm's tier / active_modules (see config/modules.config —
// 'campaigns' is in PRACTICE_ONLY_MODULE_IDS).
//
...
export const CAMPAIGNS_ALLOWED_EMAILS = [
  'christos@marnerosmarcus.co.uk',
  'christos@mmandco.com',
];

export function canAccessCampaigns(email?: string | null): boolean {
  if (!email) return false;
  const e = email.trim().toLowerCase();
  return CAMPAIGNS_ALLOWED_EMAILS.some(a => a.toLowerCase() === e);
}
