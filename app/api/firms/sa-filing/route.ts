...
import { getSaFilingStatus } from '@/lib/hmrc-sa/getSaCredsForFirm';

// Per-firm HMRC SA100 (legacy GovTalk) filing credentials.
...
export async function PATCH(request: NextRequest) {
  ...
  const { error: updateError } = await service.from('firms').update({
    sa_gateway_sender_id: parsed.data.senderId,
    sa_gateway_password_enc: encryptSecret(parsed.data.password),
  }).eq('id', firmId!);
  if (updateError) {
    console.error('PATCH /api/firms/sa-filing', updateError);
...
export async function DELETE() {
  ...
  const service = createServiceClient();
  const { error: updateError } = await service.from('firms').update({
    sa_gateway_sender_id: null,
    sa_gateway_password_enc: null,
  }).eq('id', firmId!);
  if (updateError) {
    console.error('DELETE /api/firms/sa-filing', updateError);
