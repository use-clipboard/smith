...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await service
    ...
    .select('id, business_id, type_of_business, tax_year, period_to, payload, hmrc_status, hmrc_response, submitted_at, submitter:users!mtd_it_submissions_submitted_by_fkey(full_name, email)')
    .eq('quarter_id', params.id)
    .order('submitted_at', { ascending: false });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  // Extract a human-facing HMRC reference + the filed income/expense totals.
  const submissions = (data ?? []).map(s => {
    ...
    const pay = (s.payload ?? {}) as { periodIncome?: Record<string, number>; periodExpenses?: Record<string, number> };
    const income = Object.values(pay.periodIncome ?? {}).reduce((a, v) => a + (Number(v) || 0), 0);
    const expenses = Object.values(pay.periodExpenses ?? {}).reduce((a, v) => a + (Number(v) || 0), 0);
    return {
      id: s.id,
