...
import { buildSa100ExtractPrompt } from '@/prompts/tax-studio-extract';

export const dynamic = 'force-dynamic';
export const maxDuration = 120;

// A file arrives as base64 (PDF/image) or extracted text (CSV/plain, done
// client-side). Mirrors the MTD IT analyse file shape.
...
// POST /api/tax-studio/extract — read the taxpayer's documents and return the
// SA100-relevant figures. Nothing is persisted; the client reviews then imports.
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      system: 'You are an expert UK personal-tax accountant. Always respond with valid JSON only — no prose, no code fences.',
      messages: [{ role: 'user', content: [...fileBlocks, { type: 'text', text: buildSa100ExtractPrompt(body.taxYear) }] }],
    });
    ...
    catch { return NextResponse.json({ error: 'Could not read the documents — please try again or enter the figures manually.' }, { status: 502 }); }
    ...
    console.error('[/api/tax-studio/extract]', err);
