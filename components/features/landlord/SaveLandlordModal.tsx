...
export default function SaveLandlordModal({
  isOpen,
  income,
  expenses,
  adjustments,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setClientCode(initialClient?.client_ref ?? initialClientCode ?? '');
      setStatus('idle');
      setErrorMsg('');
      setDriveCount(0);
      setDriveLinks({});
    }
  }, [isOpen, initialClient, initialClientCode]);
  ...
  const handleSave = async () => {
    ...
    if (useDrive) {
      try {
        ...
        for (const f of uploadedFiles) { if (f.driveUrl) links[f.name] = f.driveUrl; }
        setDriveLinks(links);
        ...
        if (vaultModuleActive && uploadedFiles.length > 0) {
          ...
          fetch('/api/vault/save-from-analysis', {
            ...
            body: JSON.stringify({
              ...
              sourceTool: 'landlord_analysis',
            }),
          }).catch(err => console.error('[SaveLandlordModal] vault save failed:', err));
        }
      } catch (err) {
        setStatus('error');
    ...
    exportLandlordWorkbook({
      ...
      flaggedIncome,
      flaggedExpenses,
      clientName: client?.name ?? initialClientName ?? '',
      clientCode: clientCode.trim(),
      dateFrom,
      dateTo,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--text-muted)]">
                {income.length} income · {expenses.length} expense · {documentFiles.length} source file{documentFiles.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
          {!busy && status !== 'done' && (
