...
interface ApprovalRow {
  ...
  prepared_at: string;
  /** Null until the email really goes out — see 'preparing' below. */
  sent_at: string | null;
...
export default function LandlordApprovalPanel({
  ...
  const requested = rows.filter(r => r.status !== 'preparing').length;
    ...
    <div className="glass-solid rounded-xl overflow-hidden">
      <div className="flex items-center justify-between gap-3 flex-wrap px-5 py-3.5 border-b border-[var(--border)]">
        <div className="flex items-center gap-2">
          ...
          {requested > 0 && (
            <span className="text-xs text-[var(--text-muted)]">{approved} of {requested} approved</span>
          )}
        </div>
        <button onClick={onSend} disabled={!outputId} className="btn-primary text-xs py-1.5 disabled:opacity-50">
          <Send size={12} /> {requested > 0 ? 'Send again' : 'Send for approval'}
        </button>
      </div>
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-[var(--text-muted)] mt-0.5">
                        {r.sent_at ? `Sent ${fmtDateTime(r.sent_at)}` : `Drafted ${fmtDateTime(r.prepared_at)}`}
                      </p>
                    </div>
                    <span className="inline-flex items-center gap-1.5 text-xs font-medium shrink-0">
                      ...
                      {r.status === 'preparing' && <><PenLine size={13} className="text-[var(--text-muted)]" /><span className="text-[var(--text-muted)]">Not sent yet</span></>}
                    </span>
                  </div>
                  {r.status === 'preparing' && (
