...
const SOFTWARE_BADGE: Record<TargetSoftware, string> = {
  ...
  freeagent:  'bg-rose-100 text-rose-700',
...
interface ColumnConfig {
  key: 'date' | 'user' | 'client' | 'mode' | 'tx_count' | 'period' | 'file_count' | 'files';
  label: string;
...
const COLUMNS: ColumnConfig[] = [
  ...
  { key: 'period',     label: 'Period',       defaultVisible: true },
  { key: 'file_count', label: '# Files',      defaultVisible: false },
  { key: 'files',      label: 'Source files', defaultVisible: false },
];

const COLUMN_PREF_KEY = 'smith.full-analysis.history.columns';
...
// ── Component ──────────────────────────────────────────────────────────────
export default function FullAnalysisHistory({ currentUserId, isAdmin, onNew, onOpen }: Props) {
  ...
  }, [search, software, mineOnly, dateFrom, dateTo, sortKey, sortDir]);

  useEffect(() => { void load(); }, [load]);

  // Client-side search refinement (so search is responsive without re-hitting the API for every keystroke)
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      if (SOFTWARE_LABELS[r.target_software]?.toLowerCase().includes(q)) return true;
  ...
  const handleDownload = async (id: string) => {
    ...
    try {
      ...
      const txs = output?.result_data?.transactions as Record<string, unknown>[] | undefined;
      if (!txs || txs.length === 0) {
        alert('This analysis has no transactions to download.');
        return;
      }
  ...
  const handleOpen = async (id: string) => {
    ...
    try {
      ...
      onOpen({
        id: output.id,
        targetSoftware: output.target_software,
        client: output.client,
        transactions: output.result_data?.transactions ?? [],
        flaggedEntries: output.result_data?.flaggedEntries ?? [],
        dateFrom: output.result_data?.dateFrom ?? '',
        dateTo: output.result_data?.dateTo ?? '',
      });
    } catch (e) {
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      ...
      // Fetch sequentially so we don't hammer the API; each emits its own download.
      for (const id of ids) {
        const res = await fetch(`/api/outputs/${id}`);
        if (!res.ok) continue;
        const { output } = await res.json();
        ...
        const dateStr = new Date(output.created_at).toISOString().slice(0, 10);
        const clientPart = output.client?.client_ref || output.client_name || 'analysis';
        const filename = `${clientPart}_${output.target_software}_${dateStr}.csv`;
        exportToCsv(txs, filename, output.target_software);
        // Small delay so the browser actually fires each download instead of dropping them
  ...
  const totalColSpan = visibleColCount + 2; // + select + actions
    ...
    <ToolLayout title="Full Transaction Analysis" icon={FileSearch} wide>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            placeholder="Search by client, user, mode, or filename…"
          ...
          {search && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-[var(--bg-nav-hover)] text-[var(--text-muted)]"
            >
              <X size={13} />
            </button>
          )}
      ...
      {showFilters && (
        <div className="mb-4 p-4 bg-[var(--bg-nav-hover)] border border-[var(--border)] rounded-xl flex flex-wrap items-end gap-3">
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)] mb-1">Mode</label>
            <select
              value={software}
              onChange={e => setSoftware(e.target.value)}
              className="input-base text-sm h-9 min-w-[150px]"
            >
              <option value="">All modes</option>
              {Object.entries(SOFTWARE_LABELS).map(([k, v]) => (
          ...
          {hasActiveFilters && (
            <button
              onClick={() => { setSoftware(''); setDateFrom(''); setDateTo(''); setMineOnly(false); }}
          ...
          {bulkConfirm ? (
            <>
              ...
              <button
                ...
                disabled={bulkBusy}
                className="px-3 py-1 text-xs bg-red-600 text-white rounded-full hover:bg-red-700 disabled:opacity-50 inline-flex items-center gap-1.5"
              >
              ...
              <button
                ...
                disabled={bulkBusy}
                className="px-3 py-1 text-xs border border-[var(--border)] text-[var(--text-muted)] rounded-full hover:bg-white"
              >
                Cancel
              </button>
            </>
            ...
            <>
              <Tooltip label="Download selected as CSVs">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-white border border-[var(--border)] text-xs font-medium text-[var(--text-secondary)] hover:text-emerald-600 hover:border-emerald-300 disabled:opacity-50"
                >
              ...
              <Tooltip label="Delete selected (admin or owner only)">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-white border border-[var(--border)] text-xs font-medium text-[var(--text-secondary)] hover:text-red-600 hover:border-red-300 disabled:opacity-50"
                >
              ...
              <button
                ...
                className="inline-flex items-center gap-1 h-8 px-2.5 rounded-full text-xs font-medium text-[var(--text-muted)] hover:bg-white"
              >
                <X size={12} /> Clear
              </button>
            </>
            ...
            <thead className="bg-gray-50 border-b border-[var(--border)]">
              <tr>
                <th className="px-3 py-3 w-9">
                  ...
                  >
                    {allVisibleSelected && (
                      <svg className="h-2.5 w-2.5 text-white" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="2,6 5,9 10,3" /></svg>
                    )}
                    {someVisibleSelected && !allVisibleSelected && (
                      <div className="w-2 h-0.5 bg-[var(--accent)] rounded" />
                    )}
                  </button>
                </th>
                ...
                {colVisible('mode')       && <SortHeader label="Mode" k="target_software" />}
                {colVisible('tx_count')   && <SortHeader label="# Tx" k="transaction_count" right />}
                {colVisible('period')     && <StaticHeader label="Period" />}
            ...
            <tbody>
              ...
              {!loading && !error && filteredRows.length === 0 && (
                <tr><td colSpan={totalColSpan} className="text-center py-12">
                  <FileSearch size={28} className="text-gray-300 mx-auto mb-3" />
                  <p className="text-sm text-[var(--text-muted)] mb-4">
                    {hasActiveFilters || search ? 'No analyses match your filters.' : 'No analyses yet. Run your first analysis to get started.'}
                  </p>
              ...
              {!loading && !error && filteredRows.map(row => {
                ...
                return (
                  <tr
                    key={row.id}
                    className={`border-b border-gray-100 last:border-0 transition-colors ${isSelected ? 'bg-[var(--accent-light)]/40' : 'hover:bg-indigo-50/30'}`}
                  >
                      ...
                      <td className="px-4 py-3 max-w-[220px]">
                        {row.client ? (
                          <div className="min-w-0">
                            ...
                            {row.client.client_ref && (
                              <span className="text-[10px] font-mono text-gray-400">{row.client.client_ref}</span>
                            )}
                          </div>
                        ) : row.client_name ? (
                    ...
                    {colVisible('tx_count') && (
                      <td className="px-4 py-3 text-right tabular-nums text-sm text-[var(--text-secondary)]">
                      ...
                      {confirming ? (
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            disabled={isBusy}
                            className="px-2.5 py-1 text-xs bg-red-600 text-white rounded-full hover:bg-red-700 disabled:opacity-50"
                          >
                            {isBusy ? <Loader2 className="h-3 w-3 animate-spin inline" /> : 'Yes'}
                          </button>
                          <button
                            onClick={() => setConfirmDeleteId(null)}
                            className="px-2.5 py-1 text-xs border border-[var(--border)] text-[var(--text-muted)] rounded-full hover:bg-[var(--bg-nav-hover)]"
                          >
                            No
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Tooltip label="Download spreadsheet">
                            <button
                              ...
                              className="h-7 w-7 rounded-full inline-flex items-center justify-center text-[var(--text-muted)] hover:text-emerald-600 hover:bg-emerald-50 disabled:opacity-50 transition-colors"
                            >
                          ...
                          {canDelete && (
                            <Tooltip label="Delete">
                              <button
                                ...
                                className="h-7 w-7 rounded-full inline-flex items-center justify-center text-[var(--text-muted)] hover:text-red-600 hover:bg-red-50 transition-colors"
                              >
                                <Trash2 size={13} />
                              </button>
                            </Tooltip>
