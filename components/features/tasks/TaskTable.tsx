        ...
        >
          ...
          <thead>
            <tr>
              {columns.map(c => {
                const w = widths[c.id] ?? c.defaultWidth;
                // Opaque fill (not translucent) so rows are fully hidden as they
                // scroll under the pinned header instead of bleeding through.
                const baseTh = 'sticky top-0 z-10 bg-[#f4f5fa] border-b border-[var(--border)] px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide select-none';
