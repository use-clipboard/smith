...
import { Building2, Eye, EyeOff, CheckCircle2, AlertCircle, Trash2, Info } from 'lucide-react';
import Tooltip from '@/components/ui/Tooltip';

// Settings → Tax Studio: the firm's HMRC Government Gateway "Corporation Tax for
// agents" credentials, used to file CT600 returns via HMRC's Corporation Tax
// Online service. A separate login from the SA100 credentials above. The password
// is stored encrypted server-side and never returned here.
export default function CtFilingSettings() {
  ...
  async function doSave() {
    ...
    try {
      const res = await fetch('/api/firms/ct-filing', {
        method: 'PATCH',
  ...
  async function doRemove() {
    ...
    try {
      const res = await fetch('/api/firms/ct-filing', { method: 'DELETE' });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
  ...
  return (
    <div className="glass-solid rounded-xl p-6 space-y-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">HMRC CT600 filing credentials</h3>
          <p className="text-xs text-[var(--text-muted)] mt-0.5">
            Tax Studio files Corporation Tax (CT600) returns to HMRC for your company clients. Enter your firm’s HMRC <strong>Government Gateway “Corporation Tax for agents”</strong> sign-in — a separate login from the SA100 credentials above.
          </p>
        </div>
      </div>
      ...
      <div className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm ${
        ...
        {ready ? (
          <><CheckCircle2 size={15} className="shrink-0" /> <span>Filing credentials configured{source === 'env' ? ' (system default)' : senderId ? ` — User ID ${senderId}` : ''} — CT600 online filing is enabled.</span></>
        ) : hasCreds ? (
          <><AlertCircle size={15} className="shrink-0" /> <span>Credentials saved, but CT600 online filing isn’t active yet — see the note below.</span></>
        ) : (
          <><AlertCircle size={15} className="shrink-0" /> <span>No filing credentials set — CT600 online filing is disabled until you add them.</span></>
        )}
      </div>
      ...
      {!vendorOk && (
        <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg text-xs bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800/30 text-rose-700 dark:text-rose-400">
          ...
          <span>The HMRC CT Vendor ID isn’t configured on the server yet, so filing can’t proceed even with credentials set. Please contact support.</span>
        </div>
      )}
        ...
        {pendingAction && (
          <div className={`rounded-xl border px-4 py-3 ${pendingAction === 'remove' ? 'border-rose-200 bg-rose-50/70' : 'border-amber-200 bg-amber-50/70'}`}>
            <p className="text-[12.5px] font-semibold text-[var(--text-primary)]">
              {pendingAction === 'remove' ? 'Remove the firm’s HMRC CT600 filing credentials?' : 'Save your HMRC CT600 filing credentials?'}
            </p>
            <p className="mt-0.5 text-[11.5px] text-[var(--text-muted)]">
              {pendingAction === 'remove' ? 'CT600 online filing will stop working until new credentials are added. ' : ''}
              SMITH will refresh to apply this — any work in progress may be lost.
            </p>
      ...
      <div className="pt-1 border-t border-[var(--border)] space-y-2">
        <p className="text-xs text-[var(--text-muted)] flex items-start gap-1.5">
          ...
          <span>Your password is stored encrypted and is never shown again or sent to your browser.</span>
        </p>
      </div>
    </div>
  );
