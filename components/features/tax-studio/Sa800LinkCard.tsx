...
interface TradingSummary { found: boolean; source: 'as' | 'bk'; label: string; periodStart?: string; periodEnd?: string; turnover: number; expenses: number; netProfit: number }
...
export default function Sa800LinkCard({ clientId, taxYear, existingPartners, onImportTrading, onImportPartners, onPeriod }: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!t) {
        try {
          const bk = await fetchJson<{ found: boolean; bookName?: string; from?: string; to?: string; turnover?: number; totalExpenses?: number; netProfit?: number }>(`/api/tax-studio/integrations/bookkeeping?clientId=${clientId}&taxYear=${encodeURIComponent(taxYear)}`, { cache: 'no-store' });
          if (bk.found) t = { found: true, source: 'bk', label: bk.bookName || 'Bookkeeping', periodStart: bk.from, periodEnd: bk.to, turnover: bk.turnover ?? 0, expenses: bk.totalExpenses ?? 0, netProfit: bk.netProfit ?? 0 };
        } catch { /* none */ }
      }
      let ps: PartnerRow[] = [];
      try {
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {trading && (
          <div className="flex items-center gap-2.5 rounded-lg border border-[var(--border)] bg-white/70 px-3 py-2">
            ...
            <button onClick={importTrading} disabled={importedT} className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-[var(--accent)] px-2 py-1 text-[11px] font-semibold text-white disabled:opacity-40">
              {importedT ? <><CheckCircle2 size={12} /> Filled</> : <><Download size={12} /> Fill trading</>}
            </button>
          </div>
        )}
        {partners.length > 0 && (
