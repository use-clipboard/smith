...
export interface SummariseHistoryRow {
  id: string;
  feature: string;
...
export interface SummariseSeed {
  id: string;
  client: HistoryClient | null;
  documents: OutOfRangeDocument[];
  groupBy: GroupBy;
  dateFrom: string;
  dateTo: string;
}
...
interface Props {
  ...
  isAdmin: boolean;
  onNew: () => void;
  onOpen: (seed: SummariseSeed) => void;
}

type SortKey = 'created_at' | 'client_name' | 'transaction_count';

interface ColumnConfig {
  key: 'date' | 'user' | 'client' | 'doc_count' | 'period' | 'file_count' | 'files';
  label: string;
...
const COLUMNS: ColumnConfig[] = [
  ...
  { key: 'doc_count',  label: '# Docs',       defaultVisible: true },
  { key: 'period',     label: 'Date period',  defaultVisible: true },
  { key: 'file_count', label: '# Files',      defaultVisible: false },
  { key: 'files',      label: 'Source files', defaultVisible: false },
];

const COLUMN_PREF_KEY = 'smith.summarise.history.columns';
...
// Build the same 3-sheet workbook the live tool produces, from saved data.
...
function exportSummaryWorkbook(results: OutOfRangeDocument[], filename: string) {
...
// ── Component ──────────────────────────────────────────────────────────────
export default function SummariseHistory({ currentUserId, isAdmin, onNew, onOpen }: Props) {
  const [rows, setRows]         = useState<SummariseHistoryRow[]>([]);
  ...
  // ── Re-build the 3-sheet workbook from saved data ──────────────────────
  const buildAndDownloadWorkbook = async (output: { result_data: Record<string, unknown>; client: HistoryClient | null; client_name: string | null; created_at: string }) => {
    const rd = output.result_data as { documents?: OutOfRangeDocument[] };
    const docs = rd.documents ?? [];
    if (docs.length === 0) {
      alert('This summary has no documents to download.');
      return;
    }
    const dateStr = new Date(output.created_at).toISOString().slice(0, 10);
    const clientPart = output.client?.client_ref || output.client_name || 'summary';
    const filename = `${clientPart}_summarised_${dateStr}.xlsx`;
    exportSummaryWorkbook(docs, filename);
  };
  ...
  const handleOpen = async (id: string) => {
    ...
    try {
      ...
      onOpen({
        ...
        documents: (rd.documents as OutOfRangeDocument[]) ?? [],
        groupBy: ((rd.groupBy as GroupBy) ?? 'none'),
        dateFrom: String(rd.dateFrom ?? ''),
        dateTo: String(rd.dateTo ?? ''),
      });
    } catch (e) {
          ...
          <table className="w-full">
            <thead className="bg-gray-50/80 border-b border-[var(--border)]">
              <tr>
                ...
                {colVisible('doc_count')  && <SortHeader label="# Docs" k="transaction_count" right />}
                {colVisible('period')     && <StaticHeader label="Date period" />}
              ...
              {!loading && !error && filteredRows.length === 0 && (
                <tr><td colSpan={totalColSpan} className="text-center py-12">
                  ...
                  <p className="text-sm text-[var(--text-muted)] mb-4">
                    {hasActiveFilters || search ? 'No summaries match your filters.' : 'No summaries yet. Run your first one to get started.'}
                  </p>
                  <button onClick={onNew} className="btn-primary inline-flex items-center gap-2">
